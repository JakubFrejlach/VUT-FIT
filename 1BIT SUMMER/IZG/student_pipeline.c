...
VertexIndex gpu_computeGLVertexID(
  ...
  if(indices == NULL)
	  return vertexShaderInvocation; // Indexing neni aktivni
  else
	  return indices[vertexShaderInvocation]; // Indexing je aktivni 
  return 0;
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  VertexIndex index = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation); // Vypocet indexu
  const GPUVertexPullerHead *head; // Pomocny ukazatel 
  for (int i = 0; i < MAX_ATTRIBUTES; i++)  // Vypocet pro vsechny atributy
  {
	  head = &puller->heads[i]; // Adresa hlavy ve strukture puller
	  if (head->enabled != 0) // Cteci hlava je povolena
		  output->attributes[i] = gpu_computeVertexAttributeDataPointer(head, index); // Vypocet pointeru a ulozeni do pole atributu
	  else // Cteci hlava neni povolena
		  output->attributes[i] = NULL;	// Atribut je NULL
  }
}
...
void gpu_runPrimitiveAssembly(
  ...
  GPUVertexShaderInput in; // Pomocna promenna Shader input
  GPUVertexPullerOutput out; // Pomocna promenna Puller output
  for (size_t i = 0; i < nofPrimitiveVertices; i++) // Provedeme vertex puller a vertex shader nad kazdym vrcholem primitiva
  {
	  gpu_runVertexPuller(&out, puller, baseVertexShaderInvocation + i); 
	  in.attributes = &out; // Vlozime adresu atributu
	  in.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + i); // Spocitame index
	  vertexShader(&(primitive->vertices[i]), &in, gpu); // Spustime vertex shader
  }
...
void gpu_computeScreenSpaceBarycentrics(
	...
	assert(lines != NULL);
	// vypocet barycentrickych souradnic prevzat z -> https://en.wikipedia.org/wiki/Barycentric_coordinate_system
	float determinant = (vertices[1].data[1] - vertices[2].data[1])*(vertices[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0])*(vertices[0].data[1] - vertices[2].data[1]);

	coords->data[0] = ((vertices[1].data[1] - vertices[2].data[1])*(pixelCenter->data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (pixelCenter->data[1] - vertices[2].data[1])) / determinant;
	coords->data[1] = ((vertices[2].data[1] - vertices[0].data[1])*(pixelCenter->data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0]) * (pixelCenter->data[1] - vertices[2].data[1])) / determinant;
	coords->data[2] = (float)1.0 - coords->data[0] - coords->data[1];
}
